      ...
      this.on('input', function(msg) {
        ...
        for (var i=0; i<deepProperties.length; i++) {
          ...
          if (prop.hasOwnProperty(deepProperties[i])) {
            ...
            node.log("node-red-filter: message dropped (property '" + node.property + "' not found)");
            prop = null;
          }
        }
        ...
        if (rx!=null && rx.test(prop))
          ...
          node.log("node-red-filter: message dropped (not matching the '" + node.filter + "' regular expression)");
      })
    }
